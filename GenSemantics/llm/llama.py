...
class LlamaRSEmb(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if batch_size >= 2:
            # 正常split the pair of outputs
            half_bs = batch_size // 2
            chosen_logits, rejected_logits = pooled_logits.split(half_bs, dim=0)
        else:
            # 对于单条样本，不做 split，直接作为 chosen_logits
            chosen_logits = pooled_logits
            rejected_logits = None  # 或者 pooled_logits.clone()
