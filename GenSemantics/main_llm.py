...
def train():
    ...
    def _save_item_emb():
        dataset_choice = getattr(data_args, "dataset_choice", "yelp")
        handled_dir = os.path.join("data", dataset_choice, "handled")
        ...
        output_path = os.path.join(handled_dir, "{}.pkl".format(model_args.output_file))
    ...
    if training_args.do_predict:
        ...
        if model_args.model_choice == "said":
            ...
            with open(output_prediction_file, "w", encoding="utf-8") as writer:
                for idx, samp in enumerate(list_test_samples):
                    # 防止 predictions 少于样本数
                    if idx < len(predictions):
                        p = predictions[idx]
                        samp["target"] = p.astype(float).tolist()
                    else:
                        samp["target"] = None  # 或全 0

                    if hidden_states is not None and idx < len(hidden_states):
                        samp["hidden_states"] = hidden_states[idx].astype(float).tolist()
                    else:
                        samp["hidden_states"] = None
